...
def _merge_tweets_from_txt(): # Deal with split tweets data 
    walk_dir = parent_path + '/data_figure_analysis/data/covid-id'
...
def _catch_error_tweetes():
    PATH = parent_path + '/data_figure_analysis/data/tweets-id/tweet-ids-001.txt'
    container = []
    fp = open(PATH, 'r')
    lines = fp.readlines()
    n = len(lines)
    ...
    f = open(parent_path + '/data_figure_analysis/data/tweets-id/tweet-id_filter.txt', 'w')
    for id in container:
        f.write('%s' % id)
